...
def plot_srcprofile(blob, src, sid, bands=None):
    ...
    for idx, band in zip(bidx, bands):
        ...
        if normmin != normmax:
            ...
            norm=None
        ax[0,0].imshow(img, norm=norm, cmap='Greys', extent=extent, origin = 'lower')
        ...
        ax[1,1].imshow(err, cmap='Greys', extent=extent, origin = 'lower')
        ...
        ax[1,2].imshow(mask, cmap='Greys', extent=extent, origin = 'lower')
        ...
        ax[1,3].imshow(~seg, cmap='Greys', extent=extent, origin = 'lower')
        ...
        ax[2,0].imshow(img/err, vmin=-3, vmax=3, cmap='RdGy', extent=extent, origin = 'lower')
        ...
        ax[2,1].imshow(mod, norm=norm, cmap='Greys', extent=extent, origin = 'lower')
        ...
        ax[2,2].imshow(res, vmin=-3*rms, vmax=3*rms, cmap='RdGy', extent=extent, origin = 'lower')
        ...
        ax[2,3].imshow(chi, vmin=-3, vmax=3, cmap='RdGy', extent=extent, origin = 'lower')
...
def plot_blobmap(brick, image=None, band=None, catalog=None, mode='rms'):
    ...
    if mode == 'log':
        vmin, vmax = np.nanmax([backlevel + noisesigma, 1E-5]), np.nanmax(brick.images[0])
        norm = LogNorm(np.nanmax([backlevel + noisesigma, 1E-5]), 0.9*np.nanmax(image), clip='True')
